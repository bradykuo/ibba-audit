...
/* normalise a course code for comparison */
function normCode(code) {
  return (code || "").replace(/\s+/g, "").toUpperCase();
}
/* look up a TSE course by code; returns entry or null */
function findTse(code) {
  const cc = normCode(code);
  return (CAT.tseCourses || []).find(t => normCode(t.code) === cc) || null;
}
/* per-course advanced-elective credit cap; null if uncapped */
function cappedCreditFor(code) {
  const cc = normCode(code);
  const hit = (CAT.advancedCapped || []).find(x => normCode(x.code) === cc);
  return hit ? hit.capCredits : null;
}
/* is this course barred from advanced electives? returns note or null */
function excludedFromAdvanced(code) {
  const cc = normCode(code);
  const hit = (CAT.excludeFromAdvanced || []).find(x => normCode(x.code) === cc);
...
function auditFromLines(rawLines) {
  ...
  lines.forEach(line => {
    ...
    // 1b) recognised TSE course -> advanced elective extra
    const tse = findTse(c.code);
    if (tse) {
      result.extra.push({ code: tse.code, name: tse.name, cr: tse.cr, g: c.grade });
      return;
    }
...
function renderExtra() {
  ...
  ul.innerHTML = state.extra.map((e, i) => {
    const cap = cappedCreditFor(e.code);
    const isTse = !!findTse(e.code);
    const crText = cap !== null
      ? `${e.cr} 學分（上限 ${cap}）`
      : `${e.cr} 學分`;
    return `
    <li style="display:flex;align-items:center;gap:10px;padding:6px 8px;
      ...
      <span style="flex:1;">${e.name}${isTse ? ' <span class="pill ok">TSE</span>' : ""}</span>
      <span style="color:var(--gold);font-weight:700;">${crText}</span>
...
function compute() {
  ...
  const advCounted = [];   // {code, cr, done} for exclusive-pair check
  state.extra.forEach(e => {
    let cr = Number(e.cr) || 0;
    // apply per-course cap (e.g. ISS 581700 capped at 1 advanced credit)
    const cap = cappedCreditFor(e.code);
    if (cap !== null) cr = Math.min(cr, cap);
    const isDone = e.g && e.g.trim() && !/in ?progress/i.test(e.g);
    if (isDone) extraDone += cr; else extraProg += cr;
    if (e.code) advCounted.push({ code: normCode(e.code), cr, done: isDone });
  });
  // also include table advanced courses that carry a code
  CAT.advanced.forEach((c, i) => {
    if (state.adv[i].s !== 0 && c.code)
      advCounted.push({ code: normCode(c.code), cr: c.cr, done: state.adv[i].s === 1 });
  });

  // advanced exclusive pairs: keep only the largest credit in each group,
  // deduct the rest (they become free electives)
  let pairDeductDone = 0, pairDeductProg = 0;
  const pairWarns = [];
  (CAT.advancedExclusivePairs || []).forEach(p => {
    const codes = p.codes.map(normCode);
    const members = advCounted.filter(x => codes.includes(x.code));
    if (members.length > 1) {
      // keep the one with the most credits; deduct the others
      members.sort((a, b) => b.cr - a.cr);
      members.slice(1).forEach(m => {
        if (m.done) pairDeductDone += m.cr; else pairDeductProg += m.cr;
      });
      pairWarns.push(p.note || p.codes.join(" / "));
    }
  });

  const advDone = adv.done + coreSpillCr + extraDone - pairDeductDone;
  const advProg = adv.prog + extraProg - pairDeductProg;
...
function update() {
  ...
  // advanced exclusive-pair notice
  const apw = document.getElementById("advPairWarn");
  if (apw) {
    if (c.pairWarns && c.pairWarns.length) {
      apw.style.display = "block";
      apw.innerHTML = "⚖ <b>進階擇一已套用：</b>" +
        c.pairWarns.join("；") + "（重複的部分已自動改計為自由選修，不列入進階學分）";
    } else {
...
async function init() {
  try {
    ...
    document.body.innerHTML =
      '<p style="padding:40px;font-family:sans-serif">無法載入 courses.json，' +
      '請確認檔案存在且透過網頁伺服器（非 file://）開啟。</p>';
    return;
  }
  renderAll();

  // populate TSE quick-pick datalist + semester label
  const tseList = document.getElementById("tseList");
  if (tseList && CAT.tseCourses) {
    tseList.innerHTML = CAT.tseCourses.map(t =>
      `<option value="${t.code}">${t.name}（${t.cr} 學分）</option>`).join("");
  }
  if (CAT.currentSemester) {
    const hint = document.getElementById("tseHint");
    if (hint) hint.textContent =
      `本學期（${CAT.currentSemester}）TSE 認列課程共 ${(CAT.tseCourses || []).length} 門，` +
      `於「科號」欄輸入或選取課號即自動帶入名稱與學分。`;
    const semEl = document.getElementById("semesterLabel");
    if (semEl) semEl.textContent = CAT.currentSemester;
  }
  ...
  document.getElementById("adAdd").onclick = () => {
    const codeIn = document.getElementById("adCode").value.trim();
    let name = document.getElementById("adName").value.trim();
    let cr = Number(document.getElementById("adCr").value) || 3;

    // recognise a TSE course by code -> auto-fill name & credits
    const tse = findTse(codeIn);
    if (tse) { if (!name) name = tse.name; cr = tse.cr; }

    if (!name) { document.getElementById("adName").focus(); return; }

    // block courses that cannot be advanced electives (EAP/ESP)
    const barred = excludedFromAdvanced(codeIn);
    if (barred) {
      alert("此課程不可列為進階選修：\n" + barred +
            "\n\n（如需計入，請改列自由選修。）");
      return;
    ...
    const cap = cappedCreditFor(codeIn);
    state.extra.push({
      code: codeIn, name, cr,
